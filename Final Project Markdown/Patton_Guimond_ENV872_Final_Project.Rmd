...
Benning, GA experienced 334 additional days at Black or Red flag
...
```{r, Ft Benning full year Black + Red flag analysis, echo=FALSE}

```


A second limitation is the data sets themselves. Increasing temperatures
are consequence of global warming and occur slowly. The ten year period
is hypothesized to be too small a time frame to achieve statistical
...
correlations tests, resulting in the large confidence intervals seen at
each of the installations. Confidence intervals all ranged by more than
1C and at each installation ranged from a negative to positive value.
...
that were \> 88F WBGT during the months of April - September. Ft Sill,
OK, and Ft Jackson, SC tied for the second greatest number of days under
Red or Black flag conditions at 510. Ft Sill, OK had slightly more of
the hottest Black flag days. Ft Leonard Wood, MO had the least amount of
...
Knowing the number of total Black and Red flag days at each installation
assists long range planning efforts. Using the analysis conducted in
this study, the number of days when training may be impacted during the
warmer months of each year can be used to plan for the number of extra
days built in to a quarterly or yearly training program. In this sense,
high heat days and military training planning is analogous to elementary
school districts and snow days; a set number of days are programmed into
the school year to account for those days when snow closes school. It is
only if the season is particularly bad and the number of actual days
impacted exceeds the planned estimate (for example, snow storms
exceeding snow days for school districts) that additional days need to
...
study data set, which covered only ten years, and the choice to analyze
Black and Red flag days during the period of April to September instead
of the entire year. Given the gradual progression of climate change (at
least compared to the 10 year time period used), a study using data with
a longer time frame may have found more statistically significant
conclusion.
